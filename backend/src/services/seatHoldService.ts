...
export const holdSeat = async (seatId: string, userId: string): Promise<{ success: boolean; message: string }> => {
    ...
    await holdExpiryQueue.add(
        'release-seat',
        { seatId, userId },
        { delay: HOLD_TTL_SECONDS * 1000, jobId: `hold-${seatId}` }
    );

    return { success: true, message: 'Seat held successfully' };
};

export const releaseSeat = async (seatId: string, userId?: string): Promise<void> => {
    const lockKey = `seat_lock:${seatId}`;
    await redisClient.del(lockKey);
