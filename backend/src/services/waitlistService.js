...
export const assignNextInWaitlist = async (eventId, category, seatId) => {
    ...
    nextEntry.offerSeatId = seatId;
    nextEntry.offerExpiresAt = offerExpiresAt;
    await nextEntry.save();
    // schedule offer expiry job
    await offerExpiryQueue.add('offer-expire', { waitlistId: nextEntry._id.toString(), seatId, eventId, category }, { delay: OFFER_TTL_SECONDS * 1000, jobId: `offer-${nextEntry._id}` });
