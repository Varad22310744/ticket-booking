...
export const acceptWaitlistOffer = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (entry.customer.toString() !== userId) return res.status(403).json({ message: 'Not your offer' });
        if (entry.status !== 'offered') return res.status(409).json({ message: 'Offer expired or already used' });
        ...
        const event: any = await Event.findById(entry.event);
        ...
        res.status(500).json({ message: 'Accept offer failed', error: (err as Error).message });
    }
};