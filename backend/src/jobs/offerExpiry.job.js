...
export const offerExpiryQueue = new Queue('offerExpiry', { connection: { url: redisUrl } });
const offerExpiryWorker = new Worker('offerExpiry', async (job) => {
...
export const cancelOfferExpiry = async (waitlistId) => {
    const job = await offerExpiryQueue.getJob(`offer-${waitlistId}`);
    if (job)
        await job.remove();
};
//# sourceMappingURL=offerExpiry.job.js.map